"""Project-local ByteTrack implementation.

This module owns the core tracker lifecycle so custom experiments do not rely
on Ultralytics' internal tracker registry.
"""
...
class STrack(BaseTrack):
    ...
    def __init__(self, xywh, score, cls):
        super().__init__()
        assert len(xywh) == 5, f"expected xywh+idx with 5 values, got {len(xywh)}"
        self._tlwh = xywh_to_tlwh(xywh[:4])
    ...
    def multi_predict(stracks):
        if len(stracks) == 0:
            return
        multi_mean = np.asarray([track.mean.copy() for track in stracks])
        multi_covariance = np.asarray([track.covariance for track in stracks])
...
class BYTETracker:
    def __init__(self, args):
        ...
        self.frame_id = 0
        self.args = args
        self.max_frames_lost = int(args.track_buffer)
    ...
    def update(self, results, img=None, feats=None):
        ...
        detections = self.init_track(results)
        ...
        return np.asarray([track.result for track in self.tracked_stracks if track.is_activated], dtype=np.float32)
    ...
    def init_track(self, results):
        ...
        return [STrack(xywh, score, cls) for xywh, score, cls in zip(bboxes, results.conf, results.cls)]
    ...
    def sub_stracks(tlista, tlistb):
        track_ids_b = {track.track_id for track in tlistb}
        return [track for track in tlista if track.track_id not in track_ids_b]
